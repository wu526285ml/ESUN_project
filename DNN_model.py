################################################################
# Loading Data
tr_path = 'train_data.csv'  # path to training data
...
###############################################################################
#  Testing
def save_pred(ind, preds, file):
    ''' Save predictions to specified file '''
    print('Saving results to {}'.format(file))
    with open(file, 'w') as fp:
        writer = csv.writer(fp)
        writer.writerow(['id', 'tested_positive'])
        for i, p in zip(ind.index.tolist(),preds):
            writer.writerow([i, p])

preds = test(tt_set, model, device)  # predict COVID-19 cases with your model
ind = pd.read_csv(tt_path,index_col=0)

save_pred(ind, preds, 'pred.csv')  
...
answer = pd.read_csv('test_answer_del5.csv',index_col=0)
